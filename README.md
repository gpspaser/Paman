...
<header>
  ...
  <div class="marquee">
    <p>⚠️ Info: Istirahat waktu Dzuhur-Asar-Magrib-Isya.Malam jum'at off yaa'</p>
  </div>
</header>
...
function closeModal(){
  ...
  // Ambil data sesuai jenis layanan
  ...
  if(layananAktif === 'antarkan'){
    ...
    pesan += `📍 Antar ke: ${antar}%0A`;
    pesan += `📦 Keterangan: ${ket}%0A`;
  }
  ...
  if(layananAktif === 'belikan'){
    ...
    for(let i=1; i<=10; i++){
      const item = document.querySelector(`input[placeholder="Item ${i}"]`)?.value;
      if(item) pesan += `- ${item}%0A`;
    }
  }

  if(layananAktif === 'ambilkan'){
    const ambil = document.getElementById('ambil')?.value;
    const waPenjual = document.getElementById('waPenjual')?.value;
    const cat = document.getElementById('catatan')?.value;
    pesan += `📍 Ambil di: ${ambil}%0A`;
    pesan += `📱 WA Penjual: ${waPenjual}%0A`;
    pesan += `📝 Catatan: ${cat}%0A`;
  }

  if(layananAktif === 'ngojek'){
    const jemput = document.getElementById('jemput')?.value;
    const tujuan = document.getElementById('tujuan')?.value;
    pesan += `📍 Jemput di: ${jemput}%0A`;
    pesan += `🎯 Tujuan: ${tujuan}%0A`;
  }

  if(layananAktif === 'travel'){
    const jemput = document.getElementById('jemput')?.value;
    const tujuan = document.getElementById('tujuan')?.value;
    const jml = document.getElementById('penumpang')?.value;
    const cat = document.getElementById('catatan')?.value;
    pesan += `📍 Jemput di: ${jemput}%0A`;
    pesan += `🎯 Tujuan: ${tujuan}%0A`;
    pesan += `👥 Penumpang: ${jml}%0A`;
    pesan += `📝 Catatan: ${cat}%0A`;
  }

  pesan += `%0A_Mohon segera diproses. Terima kasih 🙏_`;

  // Simpan data ke browser
  localStorage.setItem('nama', nama);
  localStorage.setItem('wa', wa);

  // Tutup modal dan buka WA
  closeModal();
...
function kirimPesanan(){
  alert('Silakan pilih layanan dulu, lalu isi form');
}
